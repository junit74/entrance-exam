...
function schoolCards(){return `<div class="school-cards">${state.data.schools.map(s=>{const t=s.snapshot?.total,st=status(s),applied=(s.snapshot?.rows||[]).filter(isApplied);return `<a href="#school/${s.id}" class="school-card${applied.length?' school-card-applied':''}" style="--school:${s.color}"><div class="card-identity"><div class="card-name"><i class="school-dot"></i>${s.shortName}</div><div class="region-text">${esc(region(s))}</div><div class="card-application">${applied.length?appliedBadge(applied[0]):''}</div></div><div class="card-number">${rate(t?.ratio)}<small>: 1</small></div><div class="card-caption">논술 전체 · 모집 ${num(t?.seats)}명</div><div class="card-time ${st.warning?'warning':''}"><i></i>${time(s.snapshot?.sourceAt)} 기준 · ${st.label}</div></a>`;}).join('')}</div>`;}
...
function trendChart(series,{years=false}={}){
  ...
  const paths=series.map(s=>{
    const observed=s.points.filter(p=>p.value!=null),dense=observed.length>40;
    let paths=[],segment=[];
