...
function essayExamInfo(s,row=null){
  ...
  return `<section class="${row?'detail-section':'info-card essay-exam'}"><${heading}>논술 출제 범위</${heading}><p>${exam.year}학년도 · 시험시간 <strong>${exam.durationMinutes}분</strong></p><div class="essay-scope-grid">${groups.map(g=>{
    ...
    return `<div class="essay-scope-group"><h4>${esc(g.label)}</h4><p class="essay-subjects">${g.subjects.map(esc).join(' · ')}</p>${hasMath?`<div class="essay-math-tags" aria-label="수학 선택과목 출제 범위">${['미적분','기하','확률과 통계'].map(v=>`<span class="pill ${g.subjects.includes(v)?'green':''}">${v} ${g.subjects.includes(v)?'포함':'제외'}</span>`).join('')}</div>`:''}${g.note?`<p>${esc(g.note)}</p>`:''}<p>문제 구성: ${esc(g.format)}</p>${!row?`<p class="essay-units">대상: ${esc(g.appliesTo)}</p>`:''}</div>`;
  }).join('')}</div>${mathNote?'<p class="essay-scope-note">‘미적분 제외’는 선택과목명 기준입니다. 수학Ⅱ에 포함된 미분·적분은 출제 범위에 포함됩니다.</p>':''}${sourceLink(exam.sourceUrl,exam.sourceLabel)}<span class="essay-verified">공식 자료 확인 ${esc(exam.verifiedAt)}</span></section>`;
}
