...
13개 대학의 자료를 로컬에서 확인하고 GitHub Pages에 게시합니다. 자동 게시 명령은 시작 시와 이후 10분마다 실행되며, 학교가 발표한 자료가 더 최신일 때 경쟁률 이력을 갱신합니다.

공개 사이트의 운영 방식은 로컬 수집 → GitHub 게시 → Pages 배포입니다. GitHub 서버에서 진학어플라이 요청이 HTTP 403으로 차단된 이력이 있어 로컬 수집을 사용합니다. 현재 로컬에서는 13개 대학 모두 정상 수집됩니다. 추가 범위와 출처는 [7개 대학 추가 검증 기록](docs/local-expansion.md)을 참고하세요.
...
GitHub Pages가 화면을 제공합니다. 이 저장소는 로컬 수집 모드로 설정했습니다. 아래 명령을 실행하면 별도 작업 폴더에서 최신 원격 이력을 가져오고, 설정된 대학을 수집하여 데이터만 커밋·게시합니다. GitHub SSH 인증과 저장소 쓰기 권한이 필요합니다.
