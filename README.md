...
경기대·수원대·한국기술교육대·한국공학대·가천대·삼육대와 강남대·중앙대 다빈치·한신대·인하대·연세대 미래·아주대·한국항공대·고려대 세종·홍익대 세종, 총 15개 대학의 논술 경쟁률을 비교하는 개인용 대시보드입니다. **같은 코드와 JSON 데이터로 로컬 서버 또는 GitHub Actions + Pages에서 실행**합니다.

**공개 대시보드: https://junit74.github.io/entrance-exam/**

로컬 Mac이 약 10분마다 15개 대학의 자료를 수집하고 GitHub에 게시합니다. GitHub Actions는 push된 자료를 빌드하여 GitHub Pages에 배포합니다. 학교가 발표한 자료가 더 최신일 때만 경쟁률 이력을 갱신합니다. 자동 갱신을 계속하려면 Mac이 켜져 있어야 합니다.
...
- 대학별 지역·캠퍼스 표시 및 지역명 검색, 자연과학 제외, 낮은 경쟁률 기본 정렬·공학 우선 정렬 옵션, 계열·검색 필터와 방문자 공용 관심 목록
- 전체 비교의 대학별 세로 막대그래프(X축 대학·Y축 경쟁률), 대학별 화면의 경쟁률 추이 및 수집 이후 시간별 그래프, 2024~2026 최종 경쟁률
- 모집단위 비교의 경쟁률은 현재 필터 안에서 높은 순으로 상위 10%(붉은색), 상위 10~30%(주황색), 나머지(파란색)를 표시합니다. 같은 경쟁률은 같은 구간에 포함하며 정렬 순서와 무관합니다.
...
- 15개 대학의 2027 논술 출제 범위·시험시간·문제 구성과 미적분/기하/확률과 통계 포함 여부. 대학 상세에서는 유형별로, 모집단위 상세에서는 해당 범위만 표시합니다.
...
별(☆)로 등록·해제한 관심 목록은 모든 방문자가 공유합니다. 로그인 없이 PC·휴대폰·로컬 사이트·GitHub Pages에서 같은 목록을 사용하며, 화면이 열려 있으면 약 5초마다 변경을 확인합니다. 목록은 경쟁률 수집 및 GitHub 배포와 별개의 영구 온라인 저장소(Sites D1)에 저장됩니다.

기존 `essay-favorites` 브라우저 목록은 각 브라우저에서 처음 연결될 때 한 번 공용 목록에 합쳐집니다. 다른 방문자가 이미 해제한 항목은 오래된 목록으로 되살리지 않습니다. 기존 브라우저 값은 백업으로 남깁니다. 연결 실패 시 화면에 오류를 표시하며 저장 성공으로 처리하지 않습니다.

[공용 저장 API](https://entrance-exam-favorites.dydtjs75.chatgpt.site/api/favorites)를 사용합니다. 프런트엔드에는 비밀키가 없고, API는 등록된 모집단위의 추가·해제만 받습니다. 별도 API 소스는 로컬의 `../entrance-exam-favorites` 프로젝트 및 해당 Sites 소스 저장소에 보관됩니다. 새로운 모집단위는 공개 `data/catalog.json`에 반영되면 API에 최대 5분 이내 인식됩니다.
